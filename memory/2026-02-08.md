...
- **Before:** Gemini 2.5 Pro (primary) → Claude (fallback)
- **After:** Claude Sonnet 4.5 (primary) → Gemini (fallback)
...
- **Critical fix:** Added `heartbeat` config to `openclaw.json` — cron jobs with `sessionTarget: main` rely on heartbeat to deliver system events. Without heartbeat config, events queue but never fire.
...
3. **Not resolved until confirmed:** "I'll handle it" ≠ resolved - track until confirmed
...
- Don't rely on "I'll mention it later" — that's ephemeral memory
...
### Rules Added Today
- Tax documents → TRACK_FOR_ACCOUNTANT
- Serasa/SABESP → FORWARD_TO_NATALINO_AND_TRACK (Brazil water bills, goal: 6 months clear)

### Google Tasks Integration
- Bills with due dates → Create task in Tars-Personal with due date
- Work items → Create task in Tars-Work
...
### Urgency Levels
- 🔴 **Critical** - Due today or past due → Notify immediately
- 🟡 **Soon** - Due within 3 days → Notify once
- 🟢 **Watching** - No due date → Weekly digest
...
- **Situation:** Old water account from when Regis lived in São Paulo
- **Moved out:** 2015, but name still on the bill
- **Current resident:** Natalino (homeowner)
- **Problem:** Natalino sometimes fails to pay → Regis's name gets flagged by Serasa (credit bureau)
...
- **Current flow:** Request quotes via iMessage → Suppliers respond via email
